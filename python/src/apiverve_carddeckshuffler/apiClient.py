...
class CarddeckshufflerAPIClientError(Exception):
...
class ValidationError(CarddeckshufflerAPIClientError):
...
class CarddeckshufflerAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"decks": {"type": "integer", "required": False, "min": 1, "max": 10, "default": 1}, "jokers": {"type": "boolean", "required": False, "default": False}, "method": {"type": "string", "required": False, "default": "fisher-yates"}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the CarddeckshufflerAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises CarddeckshufflerAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/carddeckshuffler'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise CarddeckshufflerAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[CarddeckshufflerAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns CardDeckShufflerData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises CarddeckshufflerAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise CarddeckshufflerAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise CarddeckshufflerAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise CarddeckshufflerAPIClientError(f"Request failed: {str(e)}")
