...
class CarddeckshufflerValidationException implements Exception {
  final List<String> errors;

  CarddeckshufflerValidationException(this.errors);

  @override
  String toString() => 'CarddeckshufflerValidationException: ${errors.join("; ")}';
}
...
/// Card Deck Shuffler API client.
///
/// For more information, visit: https://apiverve.com/marketplace/carddeckshuffler?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [decks] - Number of decks to shuffle [min: 1, max: 10]
/// * [jokers] - Include jokers in the deck
/// * [method] - Shuffling method
class CarddeckshufflerClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'decks': ValidationRule(type: 'integer', required: false, min: 1, max: 10),
    'jokers': ValidationRule(type: 'boolean', required: false),
    'method': ValidationRule(type: 'string', required: false),
  };

  CarddeckshufflerClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/carddeckshuffler',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CarddeckshufflerValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CarddeckshufflerValidationException(errors);
    }
  }

  /// Execute a request to the Card Deck Shuffler API.
  ///
  /// Parameters are validated before sending the request.
  Future<CarddeckshufflerResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CarddeckshufflerException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CarddeckshufflerResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CarddeckshufflerException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CarddeckshufflerException('Resource not found');
      } else {
        throw CarddeckshufflerException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CarddeckshufflerException) rethrow;
      throw CarddeckshufflerException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Card Deck Shuffler API client.
class CarddeckshufflerException implements Exception {
  final String message;

  CarddeckshufflerException(this.message);

  @override
  String toString() => 'CarddeckshufflerException: $message';
}
